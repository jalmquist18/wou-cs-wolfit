...
def test_post_vote_count_goes_down_after_downvote(client, test_user, single_post):
    assert single_post.vote_count == 0
...
def test_comments_can_only_be_downvoted_once(client, test_user, single_post_with_comment):
    comment = single_post_with_comment.comments[0]
    new_user = User(username="robot", email="robot@gmail.com")
    db.session.add(new_user)
    db.session.commit()
